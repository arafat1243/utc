    ...
    <Layout title="Edit - Client">
        <v-container>
            <v-row no-gutters justify="center">
                <v-col cols="10">
                    <v-card flat>
                        <v-alert class="grey lighten-4 primary--text text-h4"><inertia-link :href="$route('clients.index')" class="mdi mdi-arrow-left"></inertia-link>Edit Client<v-icon large class="mr-3" color="primary">mdi-plus</v-icon></v-alert>
                        <v-form ref="serviceForm" @submit.prevent="submit" method="post" enctype="multipart/form-data">
                            <v-row justify="center">
                                ...
                                <v-col cols="12" md="7">
                                    ...
                                    <v-file-input @change="setPreview"  show-size counter accept="image/*"  v-model="form.banner_img" label="Choose Company Logo"></v-file-input>
                                </v-col>
                                <v-col cols="12">
    ...
    methods:{
        submit(){
            if(this.$refs.serviceForm.validate() && this.form.details){
              ...
              this.$inertia.post(this.$route('clients.update',this.client.id), formData)
                .then(re => {
                    this.snackbar = true;
