    ...
    <Layout title="Add - Services">
        <v-container>
            <v-row no-gutters justify="center">
                <v-col cols="10">
                    <v-card flat>
                        <v-alert class="grey lighten-4 primary--text text-h4"><inertia-link :href="$route('services.index')" class="mdi mdi-arrow-left"></inertia-link>Add New Service<v-icon large class="mr-3" color="primary">mdi-plus</v-icon></v-alert>
                        <v-form ref="serviceForm" @submit.prevent="submit" method="post" enctype="multipart/form-data">
                            <v-row justify="center">
                                ...
                                <v-col cols="12" md="7">
                                    ...
                                    <v-file-input @change="setPreview" required :rules="imageRules" show-size counter accept="image/*"  v-model="form.banner_img" label="Choose banner image"></v-file-input>
                                </v-col>
                                <v-col cols="12">
    ...
    methods:{
        submit(){
            if(this.$refs.serviceForm.validate() && this.form.details){
              ...
              this.$inertia.post(this.$route('services.store'), formData)
                .then(re => {
                    this.snackbar = true;
