<template>
    <Layout title="Add a New Course - UTC">
        <v-container>
            <v-row no-gutters justify="center">
                <v-col cols="10">
                    <v-card flat>
                        <v-alert class="grey lighten-4 primary--text text-h4"><inertia-link :href="$route('courses.index')" class="mdi mdi-arrow-left"></inertia-link>Add New Course<v-icon large class="mr-3" color="primary">mdi-plus</v-icon></v-alert>
                        <v-form ref="courseForm" @submit.prevent="submit" method="post" enctype="multipart/form-data">
                            <v-row no-gutters >
                                ...
                                <v-spacer></v-spacer>
                                <v-col cols="12" md="5">
                                    <v-text-field @keypress="onlyNumber" :rules="[v => !!v || 'Total calss count is required',v => (v && v.length >= 2) || 'You Must input 2 characters']" v-model="form.class_count" outlined label="Total Class"></v-text-field>
                                </v-col>
                                <v-col cols="12" md="4">
                                    <v-file-input @change="setPreview" show-size counter accept="image/*" :rules="imageRules" v-model="form.banner_img" label="Choose banner image"></v-file-input>
    ...
    methods:{
        ...
        submit(){
            if(this.$refs.courseForm.validate() && this.form.details){
              ...
              this.$inertia.post(this.$route('courses.store'), formData)
                .then(re => {
                    this.snackbar = true;
